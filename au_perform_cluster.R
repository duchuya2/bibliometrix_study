au_perform <- function (M) 
{
  ...
  M$gr_cluster <- as.numeric(M$gr_cluster)
  M <- M[!is.na(M$PY), ]
  AU <- names(tableTag(M, "AU"))
  k <- length(AU)
  AU <- AU[1:k]
  df <- data.frame(Author = "NA", year = NA, gr_cluster = NA, TI = "NA", SO = "NA", 
